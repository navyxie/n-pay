...
var sendPayMessagePathName = '/payservice/sendPayMessage';
var unbindPathName = '/payservice/unbind';
var getpaymentPathname = '/payservice/getpayment';
var defaultErrMsg = "支付服务繁忙，请稍后重试！";
...
function NPAY(config) {
    //不通过new 形式实例化
    if (!(this instanceof NPAY)) {
        return new NPAY(arguments[0]);
    }
...
NPAY.prototype.pay = function(paymentConfig, orderInfo, cb) {
    ...
    orderInfo.return_url = encodeURIComponent(orderInfo.return_url);
    orderInfo.notify_url = encodeURIComponent(orderInfo.notify_url);
    ...
    _postCb(config.serverDomain + payPathname, reqData, function(err, signData) {
        var resData = {};
        if (err || !signData || (util.isObject(signData) && !signData.sign && !signData.ciphertext)) {
            console.log('NPAY->pay->error:', err, orderInfo, signData);
            return cb(err || (util.isObject(signData) && signData.msg) || defaultErrMsg);
        }
...
NPAY.prototype.query = function(paymentConfig, queryInfo, cb) {
    ...
    _postCb(config.serverDomain + queryPathname, reqData, function(err, signData) {
        var resData = {};
        if (err || !signData || (util.isObject(signData) && !signData.sign && !signData.ciphertext)) {
            return cb(err || (!signData || (util.isObject(signData) && signData.msg) || defaultErrMsg) || JSON.stringify(signData));
        }
...
NPAY.prototype.sendPayMessage = function(paymentConfig, msgInfo, cb) {
    var _this = this;
    this._fillParam(msgInfo, this.config);
    var checkPaymentConfigResult = util.checkPaymentConfig(paymentConfig);
    if (!checkPaymentConfigResult.pass) {
        return cb(checkPaymentConfigResult.msg);
    }
    var checkOrderInfoResult = util.checkSendMessageInfo(msgInfo);
    if (!checkOrderInfoResult.pass) {
        return cb(checkOrderInfoResult.msg);
    }
    var config = this.config;
    paymentConfig.action = 'sendPayMessage';
    var reqData = this._bindRequest(paymentConfig, msgInfo);
    _postCb(config.serverDomain + sendPayMessagePathName, reqData, function(err, signData) {
        var resData = {};
        if (err || !signData || (util.isObject(signData) && !signData.sign && !signData.ciphertext)) {
            console.log('NPAY->sendPayMessage->error:', err, msgInfo, signData);
            return cb(err  || "支付验证码发送失败");
        }
        var plaintext = _this.parse(signData.sign, signData.ciphertext, msgInfo.npay_sign_type);
        resData = plaintext.plaintext;
        resData.data = signData.data;
        if (!plaintext.verify) {
            return cb('验签失败', resData);
...
NPAY.prototype.unbind = function(paymentConfig, msgInfo, cb) {
    var _this = this;
    this._fillParam(msgInfo, this.config);
    var checkPaymentConfigResult = util.checkPaymentConfig(paymentConfig);
    if (!checkPaymentConfigResult.pass) {
    ...
    var checkOrderInfoResult = util.checkUnbindInfo(msgInfo);
    if (!checkOrderInfoResult.pass) {
        return cb(checkOrderInfoResult.msg);
    }
    var config = this.config;
    paymentConfig.action = 'unbind';
    var reqData = this._bindRequest(paymentConfig, msgInfo);
    _postCb(config.serverDomain + unbindPathName, reqData, function(err, signData) {
        var resData = {};
        if (err || !signData || (util.isObject(signData) && !signData.sign && !signData.ciphertext)) {
            console.log('NPAY->unbind->error:', err, msgInfo, signData);
            return cb(err  || "解绑银行卡失败");
        }
        var plaintext = _this.parse(signData.sign, signData.ciphertext, msgInfo.npay_sign_type);
        resData = plaintext.plaintext;
        resData.data = signData.data;
