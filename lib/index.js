...
NPAY.prototype.parse = function(sign,data){
	sign = decodeURIComponent(sign);
	data = decodeURIComponent(data);
	var cryptoInstance = this.config.cryptoInstance;
	var plaintext =  cryptoInstance.decrypt(data,'AES',true);
	var verify = cryptoInstance.verify(plaintext,sign,plaintext.npay_sign_type);
	return {
		verify:verify,
		data:plaintext
	}
}
NPAY.prototype.getPayment = function(sign_type,cb){
	var config = this.config;
	var reqData = this._bindRequest({action:'getpayment'},{npay_sign_type:'RSA'});
	_getCb(config.serverDomain+getpaymentPathname,reqData,cb);
}
