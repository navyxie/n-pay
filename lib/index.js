...
function _post(url,data,cb){
	request.post({url:url,body:data},cb);
}
...
function NPAY(config){
	...
	if(config.paymentConfig){
		if(!!util.isObject(config.paymentConfig)){
			throw new Error('key paymentConfig must be Object.');
		}
		this.config.paymentConfig = config.paymentConfig;
	}
	
}
NPAY.prototype.pay = function(paymentConfig,orderInfo,cb){
	if(util.isFunction(orderInfo)){
		cb = orderInfo;
		orderInfo = paymentConfig;
		paymentConfig = this.config.paymentConfig;
	}
...
NPAY.prototype.query = function(paymentConfig,queryInfo,cb){
	if(util.isFunction(queryInfo)){
		cb = queryInfo;
		queryInfo = paymentConfig;
		paymentConfig = this.config.paymentConfig;
	}
	var checkPaymentConfigResult = util.checkPaymentConfig(paymentConfig);	
	if(!checkPaymentConfigResult.pass){
