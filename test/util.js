...
describe('util', function() {
    ...
    it('#extend()', function() {
        ...
        destination.should.have.properties({
            a: 1,
            b: 2,
            c: {
                c: 3
    ...
    it('#checkPaymentConfig()', function() {
        util.checkPaymentConfig(1).should.have.properties({
            pass: false,
            msg: 'paymentConfig must be Object.'
        });
        util.checkPaymentConfig({
            a: 1
        }).should.have.properties({
            pass: false,
    ...
    it('#checkOrderInfo()', function() {
        util.checkOrderInfo(1).should.have.properties({
            pass: false,
    ...
    it('#checkQueryInfo()', function() {
        util.checkQueryInfo(1).should.have.properties({
            pass: false,
    ...
    it('#getSendMessageKey()', function() {
        util.getSendMessageKey().should.containDeep(['bank_phone', 'amount', 'idcard', 'acct_name', 'card_no']);
    });
    it('#checkSendMessageInfo()', function() {
        util.checkSendMessageInfo(1).should.have.properties({
            pass: false,
            msg: 'msgInfo must be Object.'
        });
        var keys = util.getSendMessageKey();
        var keysTip = 'msgInfo must be have key ' + keys.join(',') + '.';
        util.checkSendMessageInfo({
            a: 1
        }).should.have.properties({
            pass: false,
            msg: keysTip
